...
class ChebyshevGraphConv(nn.Module):
    ...
    def forward(self, inputs):
        ...
        """
        L = ChebyshevGraphConv.get_laplacian(
            self.adj, self.normalize
        )  # [N, N]
    ...
    def cheb_polynomial(self, laplacian):
        ...
        multi_order_laplacian = torch.zeros(
            [self.K, N, N], device=laplacian.device, dtype=torch.float
        )  # [K, N, N]
        ...
        if self.K == 1:
            ...
            if self.K == 2:
                ...
                for k in range(2, self.K):
                    multi_order_laplacian[k] = (
                        2 * torch.mm(laplacian, multi_order_laplacian[k - 1])
