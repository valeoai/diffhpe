...
import numpy as np
from diffhpe.architectures.csdi_model import diff_CSDI
from diffhpe.architectures.forecasting_ct_gcn import ForecastCTGCN
from diffhpe.data.forecasting.h36m_forecasting import H36M


class ForecastDiffModel(nn.Module):
    def __init__(self, config, device, target_dim=96, seq_len=None):
        super().__init__()
        self.device = device
        ...
        if config_diff.arch == "default":
            self.diffmodel = diff_CSDI(
                config_diff, input_dim, side_dim=self.emb_total_dim
            )
        elif config_diff.arch == "gcn":
            features_to_keep = H36M.set_dim_used(config.data.joints)
            self.diffmodel = ForecastCTGCN(
                config_diff,
                n_inputs=input_dim,
                seq_len=seq_len,
    ...
    def betas_for_alpha_bar(
        self, num_diffusion_timesteps, alpha_bar, max_beta=0.5
    ):
        # """
    ...
    def calc_loss_valid(self, observed_data, cond_mask, side_info, is_train):
        loss_sum = 0
    ...
    def set_input_to_diffmodel(self, noisy_data, observed_data, cond_mask):
        ...
        # XXX How do we condition on 2D data here?
