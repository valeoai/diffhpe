...
from diffhpe.data.forecasting import utils
import math
...
class H36M(Dataset):
    def __init__(
        ...
        self.dim_used = self.set_dim_used(joints)
            ...
            for action_idx in np.arange(len(acts)):
                ...
                if self.split <= 1:
                    for subact in [1, 2]:  # subactions
                        print(
                            f"Reading subject {subj}, action {action}, "
                            f"subaction {subact}"
                        )
                        filename = "{0}/S{1}/{2}_{3}.txt".format(
                            self.path_to_data, subj, action, subact
                        )
                        the_sequence = utils.readCSVasFloat(filename)
                        ...
                        the_sequence[:, 0:6] = 0
                        p3d = utils.expmap2xyz_torch(the_sequence)

                        # self.p3d[(subj, action, subact)] = p3d.view(
                        #   num_frames, -1).cpu().data.numpy()
                        self.p3d[key] = (
                    ...
                    the_sequence1 = utils.readCSVasFloat(filename)
                    ...
                    the_seq1[:, 0:6] = 0
                    p3d1 = utils.expmap2xyz_torch(the_seq1)
                    # self.p3d[(subj, action, 1)] = p3d1.view(
                    #   num_frames1, -1).cpu().data.numpy()
                    self.p3d[key] = (
                    ...
                    the_sequence2 = utils.readCSVasFloat(filename)
                    ...
                    the_seq2[:, 0:6] = 0
                    p3d2 = utils.expmap2xyz_torch(the_seq2)

                    # self.p3d[(subj, action, 2)] = p3d2.view(
                    #   num_frames2, -1).cpu().data.numpy()
                    self.p3d[key + 1] = (
                        p3d2.view(num_frames2, -1).cpu().data.numpy()
                    )
                    # self.p3d[key + 1] = self.p3d[key + 1][:, dim_used]

                    fs_sel1, fs_sel2 = utils.find_indices_256(
    ...
    def __getitem__(self, item):
        ...
        if self.miss_type == "no_miss":
            ...
            p_miss = np.random.uniform(
                0.0, 1.0, size=[self.in_n, pose.shape[1] // 3]
            )
            p_miss_rand = np.random.uniform(0.0, 1.0)
            ...
            rand = np.random.choice(
                self.in_n,
                size=math.floor(self.miss_rate * self.in_n),
                replace=False,
            )
            right_leg = [1, 2, 3]
            for i in right_leg:
            ...
            right_leg = [1, 2, 3]
            for i in right_leg:
                mask[rand[0] : rand[0] + 10, 3 * i] = 0.0
                mask[rand[0] : rand[0] + 10, 3 * i + 1] = 0.0
                mask[rand[0] : rand[0] + 10, 3 * i + 2] = 0.0
            predict = np.zeros((self.out_n, pose.shape[1]))
            mask = np.concatenate((mask, predict), axis=0)
