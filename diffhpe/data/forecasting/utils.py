...
import torch
import os
from diffhpe.utils import forward_kinematics
...
def revert_output_format(
    poses, data_mean, data_std, dim_to_ignore, actions, one_hot
):
    """
...
def load_data_cmu_3d(
    ...
    for action_idx in np.arange(nactions):
        ...
        for examp_index in np.arange(count):
            ...
            action_sequence = readCSVasFloat(filename)
            n, d = action_sequence.shape
            exptmps = _move_to_device(
                torch.from_numpy(action_sequence).float()
            )
...
def rotmat2euler_torch(R):
    ...
    idx_spec1 = (
        (R[:, 0, 2] == 1).nonzero().cpu().data.numpy().reshape(-1).tolist()
    )
    idx_spec2 = (
        (R[:, 0, 2] == -1).nonzero().cpu().data.numpy().reshape(-1).tolist()
    )
    ...
    idx_remain = np.setdiff1d(
        np.setdiff1d(idx_remain, idx_spec1), idx_spec2
    ).tolist()
    if len(idx_remain) > 0:
...
def find_indices_256(frame_num1, frame_num2, seq_len, input_n=10):
    ...
    for _ in np.arange(0, 128):
        ...
        idxs1 = np.arange(
            idx_ran1 + 50 - input_n, idx_ran1 + 50 - input_n + seq_len
        )
        idxs2 = np.arange(
            idx_ran2 + 50 - input_n, idx_ran2 + 50 - input_n + seq_len
        )
        if idxo1 is None:
            idxo1 = idxs1
            idxo2 = idxs2
